...
$sql = "SELECT * FROM voiture v
        WHERE NOT EXISTS (
            ...
            WHERE l.Matricule = v.Matricule
              AND l.status != 'annulée'
              AND ('$today' BETWEEN l.date_debut AND l.date_fin)
        )";
...
<aside class="sidebar">
    ...
    <nav>
        <ul>
            ...
            <li class="active"><a href="acceuil.php"><i class="bi bi-car-front"></i> Gestion Voitures</a></li>
       ...
       <?php if ($result && $result->num_rows > 0): ?>
        <?php while ($row = $result->fetch_assoc()): ?>
          <tr>
            ...
            <td>
              <img src="<?= htmlspecialchars($row['photo']) ?>" alt="Photo" class="img-fluid" style="max-width: 100px;">
            </td>
            <td><?= htmlspecialchars($row['modele']) ?></td>
            <td>
                <span class="badge" style="background-color:green; color:white;">Oui</span>
            </td>
            <td>
                <a href="deletcars.php?matricule=<?= $row['Matricule'] ?>" class="btn btn-sm btn-outline-danger">
                     <i class="bi bi-trash"></i>
                </a>
                <a href="modifier.php?matricule=<?= $row['Matricule'] ?>" class="btn btn-sm btn-outline-primary">
...
<!-- Bouton Ajouter -->
<a href="addcars.php" class="add-car-btn" title="Ajouter une voiture">
