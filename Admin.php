...
if (isset($_POST['connecter'])) {
    ...
    $sql = "SELECT * FROM utilisateur WHERE loginutilisateur = '$email' AND passwordutilisateur = '$passWord' AND Roletilutilisateur=1";
    $resultat = $connexion->query($sql);
    if ($resultat && $resultat->num_rows > 0) {
            $infoAdmin = $resultat->fetch_assoc();
            $_SESSION['nom'] = $infoAdmin['nomutilisateur'];
            $_SESSION['prenom'] = $infoAdmin['prenomutilisateur'];
        ...
        <div class="col-lg-6 col-md-8">
            <div class="bg-light mb-4" style="padding: 30px;">
                <div class="text-center mb-4">
                    ...
                    <p class="text-muted">Accédez à votre espace Admin DriveNow</p>
                </div>
                <form action=" " method="POST">
                    <div class="form-group mb-3">
                        <label for="email" class="text-dark">Email</label>
                        <input type="email" class="form-control p-4 bg-white" id="email" name="email" required>
                    </div>
                    <div class="form-group mb-3">
                        <label for="password" class="text-dark">Mot de passe</label>
                        <input type="password" class="form-control p-4 bg-white"  name="password" required>
                    </div>
